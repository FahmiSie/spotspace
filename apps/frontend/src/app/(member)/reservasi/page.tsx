...
export default function ReservasiPage() {
  ...
  const filteredList = reservasiList?.filter((r) => {
    ...
    return STATUS_LABELS[r.status] === activeTab;
  });
              ...
              <div key={r.id} className="bg-white border border-stone rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:border-stone/80 transition-colors">
                <div className="flex-1 space-y-4">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold border ${r.status === 'belum_dikonfirm' && r.payment?.status === 'paid' ? 'bg-blue-50 text-blue-600 border-blue-200' : STATUS_STYLES[r.status]}`}>
                      {r.status === 'belum_dikonfirm' && r.payment?.status === 'paid' ? 'Pending Admin Approval' : r.status === 'belum_dikonfirm' ? 'Awaiting Payment' : STATUS_LABELS[r.status]}
                    </span>
                ...
                <div className="flex flex-row md:flex-col items-center md:items-end gap-3 shrink-0 border-t border-stone pt-4 md:border-t-0 md:pt-0">
                  ...
                  {r.status === "belum_dikonfirm" && r.payment?.status === 'paid' && (
                    <span className="text-sm font-medium text-stone-500 w-full md:w-auto text-center md:text-right px-2">
                  ...
                  {(r.status === "belum_dikonfirm" || r.status === "disetujui") && (
                    <Dialog>
